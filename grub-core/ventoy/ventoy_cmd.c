...
grub_file_t ventoy_grub_file_open(enum grub_file_type type, const char *fmt, ...)
{
    ...
    name = grub_file_get_vlnk(fullpath, &vlnk);
    ...
    {
        ventoy_compat_set_file_vlnk(file, vlnk);
    }
...
#ifdef GRUB_MACHINE_EFI
    data = (char *)ventoy_compat_efi_allocate_iso_buf(totlen);
#else
...
grub_ssize_t ventoy_load_file_with_prompt(grub_file_t file, void *buf, grub_ssize_t size)
{
    ...
    while (left >= VTOY_SIZE_2MB)
    {
        ...
        acc += ((grub_uint64_t)VTOY_SIZE_2MB) * 100;
        while (acc >= (grub_uint64_t)size && pct < 100)
        {
            acc -= (grub_uint64_t)size;
            pct++;
        }
        grub_printf("\r%s   %d%%    ", g_vtoy_prompt_msg, (int)pct);
        grub_refresh();  
    }
...
#ifdef GRUB_MACHINE_EFI
    buf = (char *)ventoy_compat_efi_allocate_chain_buf(file->size);
#else
...
#ifdef GRUB_MACHINE_EFI
    buf = (char *)ventoy_compat_efi_allocate_iso_buf(headlen + file->size);
#else
...
static grub_err_t ventoy_cmd_iso9660_is_joliet(grub_extcmd_context_t ctxt, int argc, char **args)
{
    ...
    if (ventoy_compat_iso9660_is_joliet())
    {
...
static grub_err_t ventoy_cmd_iso9660_nojoliet(grub_extcmd_context_t ctxt, int argc, char **args)
{
    ...
    if (args[0][0] == '1')
    {
        ventoy_compat_iso9660_set_nojoliet(1);
    }
    else
    {
        ventoy_compat_iso9660_set_nojoliet(0);
    }
...
static int ventoy_vlnk_probe_fs(ventoy_vlnk_part *cur)
{
    ...
    if (cur->dev)
    {
        cur->fs = ventoy_compat_fs_list_probe(cur->dev, fs);
    }
...
static int ventoy_collect_img_files(const char *filename, const struct grub_dirhook_info *info, void *data)
{
    ...
    else
    {
        ...
        if (ventoy_compat_dirent_size(info) == VTOY_FILT_MIN_FILE_SIZE
            || ventoy_compat_dirent_size(info) == 0)
        {
        ...
        if (img)
        {
            ...
            img->size = ventoy_compat_dirent_size(info);
            if (vlnk || 0 == img->size)
            {
                if (node->dir[0] == '/')
                {
...
static grub_err_t ventoy_cmd_img_name(grub_extcmd_context_t ctxt, int argc, char **args)
{
    ...
    {
        return grub_error(GRUB_ERR_BAD_ARGUMENT, "No such many images %ld %ld", img_id, (long)g_ventoy_img_count);
    }
...
void ventoy_fill_os_param(grub_file_t file, ventoy_os_param *param)
{
    ...
    param->vtoy_reserved[6] = ventoy_compat_get_file_vlnk(file);
...
static grub_err_t ventoy_cmd_check_secureboot_var(grub_extcmd_context_t ctxt, int argc, char **args)
{
    ...
    if (ventoy_compat_efi_secureboot_enabled())
    {
        return 0;
    }
...
static int ventoy_collect_replace_initrd(const char *filename, const struct grub_dirhook_info *info, void *data)
{
    ...
    else
    {
        ...
        {
            if (ventoy_grub_get_file_size("(%s)%s%s", pfsdir->device, pfsdir->fullpath, filename)
                > 32 * VTOY_SIZE_1MB)
            {
...
static grub_err_t ventoy_cmd_search_replace_initrd(grub_extcmd_context_t ctxt, int argc, char **args)
{
    ...
    pfsdir->dev = dev;
    pfsdir->fs = fs;
    grub_snprintf(pfsdir->device, sizeof(pfsdir->device), "%s", device_name);
    pfsdir->curpos = 1;
    pfsdir->fullpath[0] = '/';
...
static grub_err_t grub_cmd_gptpriority(grub_extcmd_context_t ctxt, int argc, char **args)
{
  ...
  {
    grub_uint32_t prio = 0;
    if (ventoy_compat_get_gpt_priority (disk, part, &prio))
      {
        grub_disk_close (disk);
        return grub_error (GRUB_ERR_BAD_PART_TABLE,
                           "Fail to read GPT attribute");
      }
    grub_snprintf (priority_str, sizeof(priority_str), "%u", prio);
  }
...
static grub_err_t grub_cmd_syslinux_nojoliet(grub_extcmd_context_t ctxt, int argc, char **args)
{
    ...
    joliet = ventoy_compat_iso9660_is_joliet();
    if (joliet == 0)
    {
        return 1;
    }
...
#ifdef GRUB_MACHINE_EFI
    ventoy_compat_efi_get_reserved_page_num(&total, &org_required, &new_required);
...
int ventoy_env_init(void)
{
    ...
    ventoy_compat_register_menu_lang_hook(ventoy_menu_lang_read_hook);
    ...
    if (g_grub_param)
    {
        g_grub_param->grub_env_get = ventoy_compat_env_get;
