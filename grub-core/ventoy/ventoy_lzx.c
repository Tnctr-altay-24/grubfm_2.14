...
static int lzx_decode ( struct lzx *lzx, struct huffman_alphabet *alphabet ) {
	...
	/* Decode symbol */
	sym = ventoy_huffman_sym ( alphabet, huf );
...
static int lzx_raw_alphabet ( struct lzx *lzx, unsigned int count,
	...
	if ( ( rc = ventoy_huffman_alphabet ( alphabet, lengths, count ) ) != 0 )
		return rc;
...
static int lzx_main_alphabet ( struct lzx *lzx ) {
	...
	/* Generate Huffman alphabet */
	if ( ( rc = ventoy_huffman_alphabet ( &lzx->main, lzx->main_lengths.literals,
				       LZX_MAIN_CODES ) ) != 0 ) {
...
static int lzx_length_alphabet ( struct lzx *lzx ) {
	...
	/* Generate Huffman alphabet */
	if ( ( rc = ventoy_huffman_alphabet ( &lzx->length, lzx->length_lengths,
				       LZX_LENGTH_CODES ) ) != 0 ) {
 ...
 */
ssize_t ventoy_lzx_decompress ( const void *data, size_t len, void *buf ) {
	struct lzx lzx;
	unsigned int i;
	int rc;
