...
grub_lua_add_menu (lua_State *state)
{
  ...
  if (n > 0)
  {
    ...
    grub_normal_add_menu_entry (n, args, NULL, NULL, NULL, NULL, NULL, p, 0, 0, NULL);
    grub_free (p);
    grub_free (args);
  }
  else
...
grub_lua_add_icon_menu (lua_State *state)
{
  ...
  if (n > 0)
  {
    ...
    grub_normal_add_menu_entry (n, args, class, NULL, NULL, NULL, NULL, p, 0, 0, NULL);
    grub_free (p);
    grub_free (args);
    grub_free (class[0]);
    grub_free (class);
...
grub_lua_add_hidden_menu (lua_State *state)
{
  ...
  if (n > 0)
  {
    ...
    if (! p)
      return push_result (state);
    hotkey = luaL_checklstring (state, 1, 0);
    grub_normal_add_menu_entry (n, args, NULL, NULL, NULL, hotkey, NULL, p, 1, 0, NULL);
    grub_free (p);
    grub_free (args);
  }
  else
...
static int
grub_lua_emit_block (struct grub_lua_blocklist_ctx *ctx,
                     grub_disk_addr_t sector, unsigned num_sectors)
{
  return grub_lua_enum_block_iter ((unsigned long long) sector << GRUB_DISK_SECTOR_BITS,
                                   (unsigned long long) num_sectors << GRUB_DISK_SECTOR_BITS,
                                   ctx->part_start, ctx->state);
}

static grub_err_t
grub_lua_read_blocklist (grub_disk_addr_t sector, unsigned offset, unsigned length,
                         char *buf __attribute__ ((unused)), void *data)
{
  struct grub_lua_blocklist_ctx *ctx = data;

  if (offset != 0 || (length & (GRUB_DISK_SECTOR_SIZE - 1)))
  ...
  if (ctx->has_pending)
    grub_lua_emit_block (ctx, ctx->start_sector, ctx->num_sectors);
...
struct grub_lua_mem_ctx
{
  grub_uint64_t total;
};

static int
grub_lua_mem_iter (grub_uint64_t addr __attribute__ ((unused)),
                   grub_uint64_t size, grub_memory_type_t type, void *data)
{
  struct grub_lua_mem_ctx *ctx = data;
...
grub_lua_enum_block (lua_State *state)
{
  const char *name;
  grub_file_t file = 0;
  struct grub_lua_blocklist_ctx ctx = {0};
  ...
  ctx.state = state;
  ctx.part_start = (lua_gettop (state) > 2) ?
                    grub_partition_get_start (file->device->disk->partition) : 0;

  file->read_hook = grub_lua_read_blocklist;
...
{
  struct grub_lua_mem_ctx mctx = {0};
  const char *str;
  grub_mmap_iterate (grub_lua_mem_iter, &mctx);
  grub_uint64_t total_mem = mctx.total;
