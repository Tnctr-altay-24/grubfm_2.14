...
static void
grubfm_sort_list (struct grubfm_enum_file_info *arr, int n)
{
  int i, j;

  for (i = 1; i < n; i++)
    for (j = i; j > 0 && list_compare (&arr[j - 1], &arr[j]) > 0; j--)
      {
        struct grubfm_enum_file_info tmp = arr[j - 1];
        arr[j - 1] = arr[j];
        arr[j] = tmp;
      }
}
...
grubfm_enum_file (char *dirname)
{
  ...
  else if (fs)
  {
    ...
    if (!disable_qsort || disable_qsort[0] != '1')
      grubfm_sort_list (ctx.file_list, ctx.nfiles);
