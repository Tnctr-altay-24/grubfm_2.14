...
static void
print_varname (grub_efi_char16_t *str)
{
  while (*str != 0x0)
    {
		   ...
		   int argc, char *argv[])
{
	...
	grub_guid_t setup_var_guid = INSYDE_SETUP_VAR_GUID;
	grub_guid_t guid;
...
"\n\n(c) 2009 by Bernhard Froemel <bfroemel@gmail.com>\n"
	...
	grub_errno = 0;
	parsed = grub_strtoul(argv[0], &endptr, 16);
	if(endptr == argv[0] || grub_errno != 0 || (endptr && *endptr) || parsed > 0xffff)
	{
		return grub_error(GRUB_ERR_BAD_ARGUMENT, "can't decode your first argument. Please provide a hex value (e.g. 0x1af).");
	}
	offset = (grub_uint16_t) parsed;

	if(argc == 2)
	{
		grub_errno = 0;
		parsed = grub_strtoul(argv[1], &endptr, 16);
		if(endptr == argv[1] || grub_errno != 0 || (endptr && *endptr) || parsed > 0xff)
		{
			return grub_error(GRUB_ERR_BAD_ARGUMENT, "can't decode your second argument. Please provide a hex value (e.g. 0x01).");
		}
		set_value = (grub_uint8_t) parsed;
	}
	...
	do
	{
		    ...
		    (isMode2 && name_size == INSYDE_CUSTOM_VAR_NSIZE && 0 == grub_memcmp(name, INSYDE_CUSTOM_VAR, name_size))))
		{
			...
			if(grub_memcmp(&guid, &setup_var_guid, sizeof(grub_guid_t)) == 0)
			{
			...
			else
			{
				...
				grub_memcpy(&setup_var_guid, &guid, sizeof(grub_guid_t));
			}
		...
		if(status)
		{
			return grub_error(GRUB_ERR_IO, "can't get variable");
		}
	...
	if(argc == 2)
	{
		...
		if(status)
		{
			return grub_error(GRUB_ERR_IO, "can't set variable");
		}
	}
	...
	if (!found)
		return grub_error (GRUB_ERR_FILE_NOT_FOUND, "setup variable not found");
	return grub_error (GRUB_ERR_FILE_NOT_FOUND, "no usable setup variable found");
}
