...
bcd_patch_dp (struct bcd_patch_data *cmd)
{
  ...
  {
    struct grub_gpt_header gpt_hdr;
    struct grub_gpt_partentry entry;

    cmd->dp.partmap = 0x00;
    if (grub_disk_read (disk, 1, 0, sizeof (gpt_hdr), &gpt_hdr) != 0
        || grub_disk_read (disk, p->offset, p->index, sizeof (entry), &entry) != 0)
      {
