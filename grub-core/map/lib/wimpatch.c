 ...
 */
static void wim_hash ( struct vfat_file *vfile, struct wim_hash *hash ) {
 ...
 */
static int wim_inject_file ( struct vfat_file *vfile ) {
  size_t name_len;
  const char *ext;
...
static int wim_patch_file ( struct wim_patch *patch __unused,
          struct wim_patch_region *region,
          void *data, size_t offset, size_t len ) {
  struct vfat_file *vfile = region->opaque;
...
static int wim_patch_lookup_file ( struct wim_patch *patch __unused,
  ...
  struct vfat_file *vfile = rfile->opaque;
  struct wim_lookup_entry entry;
...
wim_construct_region ( struct wim_patch_region *region, const char *name,
  ...
  printf ( "...patching WIM %s at [0x%lx,0x%lx)\n",
...
static size_t wim_construct_dir ( struct wim_patch *patch,
  ...
  printf ( "...patching WIM directory at 0x%lx from [0x%lx,0x%lx)\n",
...
static int wim_construct_patch ( struct vfat_file *file,
  ...
  file->xlen = offset;
  printf ( "...patching WIM length 0x%lx->0x%lx\n", file->len, file->xlen );
 ...
 * @v len    Length
 */
void patch_wim ( struct vfat_file *file, void *data, size_t offset,
     size_t len ) {
